...
df=pd.read_csv("16-Pandas/youtube-ing.csv")
...
df["tag_count"]=df["tags"].apply(lambda x: len(x.split('|'))) #1.yol

def tagCount(tag):  #2.yol
...
def LikeDislikeOran(dataset):
    ...
    liste=list(zip(likelist,dislikelist)) #(1020,200) (500,200) 1.listenin elemanları ile 2.listenin elemanları sırayla birlesir
...
df["begeni_oranı"]=LikeDislikeOran(df)
